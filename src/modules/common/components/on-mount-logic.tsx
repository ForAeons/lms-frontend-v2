...
export const OnMountLogic: React.FC = () => {
  ...
  const { status: backendFetchStatus, data: csrf } = useQuery({
    queryKey: ["health"],
  ...
  React.useEffect(() => {
    if (backendFetchStatus === "success") {
      dispatch(appSlice.actions.setCsrfToken(csrf.data));
    }
  }, [dispatch, backendFetchStatus, csrf]);

  const { status: userFetchStatus, data: userData } = useQuery({
  ...
  React.useEffect(() => {
    if (userFetchStatus !== "success" || !userData) {
      dispatch(appSlice.actions.setSignout());
    } else if (userData.data.is_logged_in) {
      dispatch(appSlice.actions.setSignin(userData.data));
    }
  }, [dispatch, userFetchStatus, userData]);
